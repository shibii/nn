...
namespace cereal {
  ...
  template <class Archive> void load(Archive& archive, af::array& a) {
  ...
  template <class Archive> void save(Archive& archive, af::dim4 const& d) {
    std::vector<dim_t> dims{ d[0], d[1], d[2], d[3] };
    archive(dims);
  }

  template <class Archive> void load(Archive& archive, af::dim4& d) {
    std::vector<dim_t> dims;
    archive(dims);
